        ...
        <form className="panel question-form" onSubmit={addQuestion}>
          ...
          {form.options.map((option, index) => (
            <input
              ...
              value={option}
              onChange={(e) => setForm({ ...form, options: form.options.map((item, i) => i === index ? e.target.value : item) })}
              required
            />
          ))}
      ...
      <section className="panel">
        ...
        <div className="cluster-list">
          {analytics?.clusters?.length
            ? analytics.clusters.map((item) => <span key={item.user_id}>Student {item.user_id}: {item.cluster}</span>)
            : <span>No cluster data yet</span>}
        </div>
      </section>
    </div>
  )

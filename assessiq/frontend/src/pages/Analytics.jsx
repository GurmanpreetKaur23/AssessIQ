...
export default function Analytics() {
  ...
  const load = async () => {
    ...
    const avgDifficulty = analytics.data.topic_stats.length ? analytics.data.topic_stats.reduce((sum, item) => sum + Math.max(1, Math.round(item.accuracy * 5)), 0) / analytics.data.topic_stats.length : 2
  ...
  const progress = useMemo(() => {
    if (!data) return []
    return data.topic_stats.map((item, index) => ({ name: item.topic, score: Math.round(item.accuracy * 100), speed: Math.round(item.avg_time), test: index + 1 }))
  }, [data])
  ...
  return (
    <div className="page">
      ...
      <section className="metric-grid">
        ...
        <div className="metric"><Timer /><span>Speed</span><strong>{data.avg_time}s</strong></div>
      ...
      <section className="insight-grid">
        <div className="panel">
          ...
          {data.insights.length ? data.insights.map((item) => <p key={item} className="insight">{item}</p>) : <p className="insight">Complete a test to generate stronger behavioral signals.</p>}
        </div>
        <div className="panel">
